...
public struct DemoPoseIntegratorCallbacks : IPoseIntegratorCallbacks
{
  ...
  public DemoPoseIntegratorCallbacks(Vector3 gravity,
    float linearDamping = .03f, float angularDamping = .03f) : this()
  {
    Gravity = gravity;
  ...
  public void PrepareForIntegration(float dt)
  {
    ...
    linearDampingDt =
      new Vector<float>(
        MathF.Pow(MathHelper.Clamp(1 - LinearDamping, 0, 1), dt));
    angularDampingDt =
      new Vector<float>(MathF.Pow(MathHelper.Clamp(1 - AngularDamping, 0, 1),
        dt));
